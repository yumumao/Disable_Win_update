...
:: 停止和禁用服务
echo [%date% %time%] 正在停止和禁用Windows更新相关服务 >> "%LogFile%"

for %%i in (wuauserv, UsoSvc, WaaSMedicSvc) do (
    echo [%date% %time%] 处理服务: %%i >> "%LogFile%"
    
    :: 停止服务
    net stop %%i >nul 2>&1
    set "stop_error=!errorlevel!"
    if !stop_error! equ 0 (
        echo [%date% %time%] %%i 服务停止成功 >> "%LogFile%"
    ) else if !stop_error! equ 2 (
        echo [%date% %time%] %%i 服务不存在或已停止 >> "%LogFile%"
    ) else (
        echo [%date% %time%] %%i 服务停止失败，错误代码: !stop_error! >> "%LogFile%"
    )
    ...
    if !config_error! equ 0 (
        echo [%date% %time%] %%i 服务禁用成功 >> "%LogFile%"
    ) else if !config_error! equ 5 (
        echo [%date% %time%] %%i 服务禁用失败（访问被拒绝），错误代码: !config_error! >> "%LogFile%"
    ) else if !config_error! equ 1060 (
        echo [%date% %time%] %%i 服务不存在，错误代码: !config_error! >> "%LogFile%"
    ) else (
        echo [%date% %time%] %%i 服务禁用失败，错误代码: !config_error! >> "%LogFile%"
    )
...
:: 通过注册表强制禁用WaaSMedicSvc（移除文件操作避免误杀）
...
if exist "C:\$WINDOWS.~BT" (
    ...
    if not exist "C:\$WINDOWS.~BT" (
        echo [%date% %time%] 升级文件夹删除成功 >> "%LogFile%"
    ) else (
        echo [%date% %time%] 升级文件夹删除失败 >> "%LogFile%"
    )
) else (
    echo [%date% %time%] 未发现升级文件夹 >> "%LogFile%"
)
...
:: 跳过计划任务禁用
echo [%date% %time%] 跳过计划任务禁用（避免卡住） >> "%LogFile%"

:: 显示完成消息
echo [%date% %time%] 检查用户会话状态 >> "%LogFile%"
set "UserLoggedIn="
...
if exist "%LogFile%" (
    ...
    if !LineCount! gtr 50000 (
        ...
        powershell -Command "(Get-Content '%LogFile%' | Select-Object -Last 25000) | Set-Content '%LogFile%_temp'" >nul 2>&1
